...
class OrdersController {
  async create(req, res) {
    const { orderItem, status, totalPrice } = req.body;
  ...
  async index(req, res) {
    ...
    if (!user.isAdmin) {
      const orders = await knex('items')
        .where({ user_id })
        .select(['orders.id', 'orders.user_id', 'orders.status', 'orders.totalPrice', 'orders.created_at'])
  ...
  async update(req, res) {
    ...
    await knex('orders').update({ status, updated_at: knex.fn.now() }).where({ id });
